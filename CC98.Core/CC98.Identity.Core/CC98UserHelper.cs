...
public static class CC98UserHelper
{
	...
	public static ClaimsIdentity ToIdentity(this CC98User userInfo, string authenticationType)
	{
		...
		{
			new(ClaimTypes.Name, userInfo.Name, ClaimValueTypes.String),
			...
			new(CC98UserClaimTypes.OldId, userInfo.V1Id.ToString("D", CultureInfo.InvariantCulture),
				ClaimValueTypes.Integer),
			new(CC98UserClaimTypes.PortraitUri, userInfo.PortraitUrl)
		};

		// 角色
		foreach (var role in userInfo.Roles)
			claims.Add(new(ClaimTypes.Role, role, ClaimValueTypes.String));
