...
public class FileUploadService
{
	...
	private async Task GetDiscoveryDocumentCoreAsync(CancellationToken cancellationToken = default)
	{
		using (var httpClient = new HttpClient())
		{
			...
			if (response.IsError) throw new InvalidOperationException(response.Error, response.Exception);
	...
	private string GetFinalScope()
	{
		...
		if (Config.RequestOfflineAccess) scopeList.Add(OidcConstants.StandardScopes.OfflineAccess);
	...
	{
		if (TokenResponse == null || TokenResponse.IsError) await GetTokenCoreAsync(cancellationToken);
	}
	...
	public async Task<IEnumerable<string>> UploadAsync(IEnumerable<UploadFileInfo> files, string subPath,
		bool compressImages, CancellationToken cancellationToken = default)
	{
		// 检查令牌。
	...
	{
		return UploadAsync(files, Config.DefaultSubPath, Config.CompressByDefault, cancellationToken);
	}
}